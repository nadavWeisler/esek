import unittest
import numpy as np
from OneSampleAparametric import AparametricOneSample
...
class TestOneSampleAparametric(unittest.TestCase):
    def test_aparametric_effect_size_onesample(self):
        ...
        results = AparametricOneSample.ApermetricEffectSizeOneSample(params)
        ...
        self.assertAlmostEqual(
            results["Wilcoxon Standard Deviation"], 28.7380, places=4
        )
        self.assertAlmostEqual(results["Wilcoxon Z"], 0.5394, places=4)
        self.assertAlmostEqual(
            results["Wilcoxon Z With Normal Approximation (Continuiety Correction)"],
            0.5220,
            places=4,
        )
        ...
        self.assertAlmostEqual(
            results[
                "Wilcoxon p-value with Normal Approximation (Continuiety Correction)"
            ],
            0.6017,
            places=4,
        )
        ...
        self.assertAlmostEqual(
            results["Matched Pairs Rank Biserial Correlation (Ignoring Ties)"],
            0.1342,
            places=4,
        )
        self.assertAlmostEqual(
            results["Z-based Rank Biserial Correlation (Wilcoxon Method)"],
            0.1177,
            places=4,
        )
        self.assertAlmostEqual(
        ...
        self.assertAlmostEqual(
            results["Pratt Z with Normal Approximation (Continuiety Correction)"],
            0.4590,
            places=4,
        )
        self.assertAlmostEqual(results["Pratt p-value"], 0.6366, places=4)
        self.assertAlmostEqual(
            results["Pratt p-value with Normal Approximation (Continuiety Correction)"],
            0.6463,
            places=4,
        )
        ...
        self.assertAlmostEqual(
            results["Matched Pairs Rank Biserial Correlation (Considering Ties)"],
            0.1077,
            places=4,
        )
        self.assertAlmostEqual(
            results["Z-based Rank Biserial Correlation (Pratt Method)"],
            0.0945,
            places=4,
        )
        self.assertAlmostEqual(
            results["Z-based Corrected Rank Biserial Correlation (Pratt Method)"],
            0.0918,
            places=4,
        )
        ...
        self.assertAlmostEqual(
            results["Lower CI Matched Pairs Rank Biserial Wilcoxon"], -0.1020, places=4
        )
        self.assertAlmostEqual(
            results["Upper CI Matched Pairs Rank Biserial Wilcoxon"], 0.3561, places=4
        )
        self.assertAlmostEqual(
            results["Lower CI Z-based Rank Biserial Wilcoxon"], -0.1186, places=4
        )
        self.assertAlmostEqual(
            results["Upper CI Z-based Rank Biserial Wilcoxon"], 0.3414, places=4
        )
