...
def Pivotal_ci_t(t_Score, df, sample_size, confidence_level):
    """
    Calculate the Pivotal Confidence Interval for a t-score.
...
def calculate_central_ci_from_cohens_d_two_paired_sample_t_test(cohens_d, sample_size, confidence_level):
    """
    Calculate the Central Confidence Interval for Cohen's d in two paired sample t-test.
    ...
    Returns:
    tuple: Lower and upper confidence intervals and standard error.
    """
...
class Two_Paired_Common_Language_Effect_Sizes():
    """
    Class for calculating Common Language Effect Sizes for two paired samples.
    """

    @staticmethod
    def Common_Language_Effect_Sizes_Paird_Samples_from_t_score(params: dict) -> dict:
        """
        Calculate Common Language Effect Sizes from t-score.
        ...
        params : dict
            ...
            - "Number of Pairs": Number of paired samples
        ...
        dict
            Dictionary containing:
            - Common language effect sizes (CLdz, CLgz)
    ...
    def Common_Language_Effect_Sizes_Paird_Samples_from_parameters(params: dict) -> dict:
        """
        Calculate Common Language Effect Sizes from sample parameters.

        Parameters
        ----------
        params : dict
            Dictionary containing:
            - "Sample Mean 1": Mean of first sample
            - "Sample Mean 2": Mean of second sample
            - "Standard Deviation Sample 1": SD of first sample
            - "Standard Deviation Sample 2": SD of second sample
            - "Number of Pairs": Number of paired samples
            - "Difference in the Population": Expected population difference
            - "Pearson Correlation": Correlation between samples
            - "Confidence Level": Confidence level in percentages
    ...
    def Common_Language_Effect_Sizes_Paird_Samples_from_data(params: dict) -> dict:
        """
        Calculate Common Language Effect Sizes from raw data.
        ...
        params : dict
            ...
            - "Confidence Level": Confidence level in percentages
            - "Number of Bootstapping Samples": Number of bootstrap iterations
...
def paired_samples_cles(x, y):
    """
    Calculate the Common Language Effect Size (CLES) for paired samples.
    ...
    -------
    float
        The Common Language Effect Size value, representing the probability that 
        a randomly selected value from one group will be greater than a randomly 
        selected value from the other group.
    """
    # ... existing code ...