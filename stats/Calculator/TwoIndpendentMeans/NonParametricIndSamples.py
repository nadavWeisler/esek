...
class Aparametric_Paired_Samples():
    ...
    def Apermetric_Effect_Size_Independent(params: dict) -> dict: #Aparametric effect size with sign test
        ...
        LowerCi_Rank_Biserial_Correlation = max(math.tanh(math.atanh(rank_biserial_correlation) - Z_Critical_Value * Standrd_Error_RBC),-1)
        UpperCi_Rank_Biserial_Correlation = min(math.tanh(math.atanh(rank_biserial_correlation) + Z_Critical_Value * Standrd_Error_RBC),1)
        LowerCi_Z_basesd = max(math.tanh(math.atanh(Rosenthal_rank_biserial_correlation_z) - Z_Critical_Value * Standrd_Error_RBC),-1)
        UpperCi_Z_basesd = min(math.tanh(math.atanh(Rosenthal_rank_biserial_correlation_z) + Z_Critical_Value * Standrd_Error_RBC),1)
        LowerCi_Z_basesd_corrected = max(math.tanh(math.atanh(Rosenthal_rank_biserial_correlation_z_corrected) - Z_Critical_Value * Standrd_Error_RBC),-1)
        UpperCi_Z_basesd_corrected = min(math.tanh(math.atanh(Rosenthal_rank_biserial_correlation_z_corrected) + Z_Critical_Value * Standrd_Error_RBC),1)
