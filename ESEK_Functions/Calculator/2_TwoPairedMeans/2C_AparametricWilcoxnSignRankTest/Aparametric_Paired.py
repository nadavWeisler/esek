...
class Aparametric_Paired_Samples():
    @staticmethod
    def Apermetric_Effect_Size_Dependent(params: dict) -> dict: #Aparametric effect size with sign test
        ...
        def sign_test_wilcoxon_method (x, y, population_median=0): #this is the original method by wilcoxon that ignores ties
            if y is None:
            ...
            Z_adjusted_Corrected = (positive_sum_ranks - MeanW - 0.5 * np.sign(positive_sum_ranks - MeanW)) /  np.sqrt(Adjusted_Variance)
            Z_Unadjusted_Corrected = (positive_sum_ranks - MeanW - 0.5 * np.sign(positive_sum_ranks - MeanW)) /  np.sqrt(Unadjusted_Variance)
            ...
            p_value_adjusted_corrected = min(float(norm.sf((abs(Z_adjusted_Corrected))) * 2), 0.99999) 
            p_value_unadjusted_corrected = min(float(norm.sf((abs(Z_Unadjusted_Corrected))) * 2), 0.99999)
            ...
            return np.array([positive_n, negative_n, total_n, positive_sum_ranks, negative_sum_ranks, total_sum_ranks, \
                            mean_positive_ranks,  mean_negative_ranks, MeanW, Adjusted_Variance, Unadjusted_Variance, RBCwilcoxon, \
        ...
        def sign_test_pratt_method (x,y, population_median=0): #this is the original method by pratt that consider ties in the data
            if y is None:
                difference = x - population_median
            else:
                difference = x - y
        ...
        wilcoxon_positive_n, wilcoxon_negative_n, wilcoxon_total_n, wilcoxon_positive_sum_ranks, wilcoxon_negative_sum_ranks, wilcoxon_total_sum_ranks, \
        wilcoxon_mean_positive_ranks, wilcoxon_mean_negative_ranks, wilcoxon_MeanW, wilcoxon_Adjusted_Var, wilcoxon_Unadjusted_Var, wilcoxon_RBCwilcoxon, \
        wilcoxon_Z_adjusted, wilcoxon_Z_Unadjusted, wilcoxon_Z_adjusted_Corrected, wilcoxon_Z_Unadjusted_Corrected, \
        wilcoxon_p_value_adjusted, wilcoxon_p_value_unadjusted, wilcoxon_p_value_adjusted_corrected, wilcoxon_p_value_unadjusted_corrected, \
        wilcoxon_RBC_adjusted, wilcoxon_RBC_unadjusted, wilcoxon_RBC_unajusted_corrected, wilcoxon_RBC_adjusted_corrected = sign_test_wilcoxon_method(column_1,column_2)

        # Second method: sign_test_pratt_method
        pratt_positive_n, pratt_negative_n, pratt_total_n, pratt_positive_sum_ranks, pratt_negative_sum_ranks, pratt_total_sum_ranks, \
        pratt_mean_positive_ranks, pratt_mean_negative_ranks, pratt_MeanW, pratt_Adjusted_Var, pratt_Unadjusted_Var, RBCpratt, \
        pratt_Z_adjusted, pratt_Z_Unadjusted, pratt_Z_adjustedCorrected, pratt_Z_Unadjusted_Corrected, \
        pratt_p_value_adjusted, pratt_p_value_unadjusted, pratt_p_value_adjusted_corrected, pratt_p_value_unadjusted_corrected, \
        pratt_RBC_adjusted, pratt_RBC_unadjusted, pratt_RBC_unajusted_corrected, pratt_RBC_adjusted_corrected = sign_test_pratt_method(column_1,column_2)

        # Confidence Intervals for the Rank Biserial Correlation
        Standrd_Error_RBR = np.sqrt(( (2*sample_size**3 + 3* sample_size**2 + sample_size)/6) / ((sample_size**2 + sample_size)/2))
        Z_Critical_Value = norm.ppf((1-confidence_level) + ((confidence_level) / 2))

        Lower_CI_Matched_Wilcoxon = math.tanh(math.atanh(wilcoxon_RBCwilcoxon) - Z_Critical_Value * Standrd_Error_RBR)
        Upper_CI_Matched_Wilcoxon = math.tanh(math.atanh(wilcoxon_RBCwilcoxon) + Z_Critical_Value * Standrd_Error_RBR)
        Lower_CI_Adjusted_Wilcoxon = math.tanh(math.atanh(wilcoxon_RBC_adjusted) - Z_Critical_Value * Standrd_Error_RBR)
        Upper_CI_Adjusted_Wilcoxon = math.tanh(math.atanh(wilcoxon_RBC_adjusted) + Z_Critical_Value * Standrd_Error_RBR)
        Lower_CI_Unadjusted_Wilcoxon = math.tanh(math.atanh(wilcoxon_RBC_unadjusted) - Z_Critical_Value * Standrd_Error_RBR)
        Upper_CI_Unadjusted_Wilcoxon = math.tanh(math.atanh(wilcoxon_RBC_unadjusted) + Z_Critical_Value * Standrd_Error_RBR)
        Lower_CI_Adjusted_Corrected_Wilcoxon = math.tanh(math.atanh(wilcoxon_RBC_unajusted_corrected) - Z_Critical_Value * Standrd_Error_RBR)
        Upper_CI_Adjusted_Corrected_Wilcoxon = math.tanh(math.atanh(wilcoxon_RBC_unajusted_corrected) + Z_Critical_Value * Standrd_Error_RBR)
        Lower_CI_Unadjusted_Corrected_Wilcoxon = math.tanh(math.atanh(wilcoxon_RBC_adjusted_corrected) - Z_Critical_Value * Standrd_Error_RBR)
        Upper_CI_Unadjusted_Corrected_Wilcoxon = math.tanh(math.atanh(wilcoxon_RBC_adjusted_corrected) + Z_Critical_Value * Standrd_Error_RBR)

        Lower_CI_Matched_Pratt = math.tanh(math.atanh(RBCpratt) - Z_Critical_Value * Standrd_Error_RBR)
        Upper_CI_Matched_Pratt = math.tanh(math.atanh(RBCpratt) + Z_Critical_Value * Standrd_Error_RBR)
        Lower_CI_Adjusted_Pratt = math.tanh(math.atanh(pratt_RBC_adjusted) - Z_Critical_Value * Standrd_Error_RBR)
        Upper_CI_Adjusted_Pratt = math.tanh(math.atanh(pratt_RBC_adjusted) + Z_Critical_Value * Standrd_Error_RBR)
        Lower_CI_Unadjusted_Pratt = math.tanh(math.atanh(pratt_RBC_unadjusted) - Z_Critical_Value * Standrd_Error_RBR)
        Upper_CI_Unadjusted_Pratt = math.tanh(math.atanh(pratt_RBC_unadjusted) + Z_Critical_Value * Standrd_Error_RBR)
        Lower_CI_Adjusted_Corrected_Pratt = math.tanh(math.atanh(pratt_RBC_unajusted_corrected) - Z_Critical_Value * Standrd_Error_RBR)
        Upper_CI_Adjusted_Corrected_Pratt = math.tanh(math.atanh(pratt_RBC_unajusted_corrected) + Z_Critical_Value * Standrd_Error_RBR)
        Lower_CI_Unadjusted_Corrected_Pratt = math.tanh(math.atanh(pratt_RBC_adjusted_corrected) - Z_Critical_Value * Standrd_Error_RBR)
        Upper_CI_Unadjusted_Corrected_Pratt = math.tanh(math.atanh(pratt_RBC_adjusted_corrected) + Z_Critical_Value * Standrd_Error_RBR)
        ...
        results["Matched Pairs Rank Biserial Correlation (Kerby, 2014)"] = wilcoxon_RBCwilcoxon
        ...
        results["Wilcoxon p-value Adjusted Corrected"] = wilcoxon_p_value_adjusted_corrected
        results["Wilcoxon p-value Unadjusted Corrected"] = wilcoxon_p_value_unadjusted_corrected
        ...
        results["Lower CI RBC Adjusted Corrected Wilcoxon"] = Lower_CI_Adjusted_Corrected_Wilcoxon
        results["Upper CI RBC Adjusted Corrected Wilcoxon"] = Upper_CI_Adjusted_Corrected_Wilcoxon
        results["Lower CI RBC Unadjusted Corrected Wilcoxon"] = Lower_CI_Unadjusted_Corrected_Wilcoxon
        results["Upper CI RBC Unadjusted Corrected Wilcoxon"] = Upper_CI_Unadjusted_Corrected_Wilcoxon
        ...
        results["Pratt p-value Unadjusted Corrected"] = pratt_p_value_unadjusted_corrected
        ...
        results["Lower CI RBC Adjusted Corrected Pratt"] = Lower_CI_Adjusted_Corrected_Pratt
        results["Upper CI RBC Adjusted Corrected Pratt"] = Upper_CI_Adjusted_Corrected_Pratt
        results["Lower CI RBC Unadjusted Corrected Pratt"] = Lower_CI_Unadjusted_Corrected_Pratt
        results["Upper CI RBC Unadjusted Corrected Pratt"] = Upper_CI_Unadjusted_Corrected_Pratt
        formatted_p_value = "{:.3f}".format(pratt_p_value_adjusted_corrected).lstrip('0') if pratt_p_value_adjusted_corrected >= 0.001 else "\033[3mp\033[0m < .001"
        results["Statistical Line"] = " \033[3mz\033[0m = {:.3f}, {}{}, \033[3mr\033[0m\u2098\u209a = {}, {}% CI(fisher) [{},{}]".format(pratt_Z_adjustedCorrected, '\033[3mp = \033[0m' if pratt_p_value_adjusted_corrected >= 0.001 else '', formatted_p_value, str(round(pratt_RBC_adjusted_corrected,9)).lstrip("0"), (confidence_level*100), str(round(Lower_CI_Matched_Pratt,3)).lstrip("0"), str(round(Upper_CI_Matched_Pratt,4)).lstrip("0"))
