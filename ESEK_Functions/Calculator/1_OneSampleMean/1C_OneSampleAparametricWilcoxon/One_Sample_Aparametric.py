...
#####################################################
## Effect Size for one Sample Aparametric Test     ##
#####################################################
...
class Aparametric_One_Sample():
    @staticmethod
    def Apermetric_Effect_Size_onesample(params: dict) -> dict: #Aparametric effect size with sign test
        ...
        positive_n = difference[difference > 0].shape[0] # How many times sample is greater than population value
        negative_n = difference[difference < 0].shape[0] # How many times sample is lower than population value
        ...
        Matched_Pairs_Rank_Biserial_Corelation_ignoring_ties = min((positive_sum_ranks_no_ties-negative_sum_ranks_no_ties)/np.sum(ranked_no_ties),0.99999999) #This is the match paired rank biserial correlation using kerby formula that is not considerign ties (Kerby, 2014)
        Matched_Pairs_Rank_Biserial_Corelation_Considering_ties = min((positive_sum_ranks_with_ties-negative_sum_ranks_with_ties)/np.sum(ranked_with_ties),0.999999999) # this is the Kerby 2014 Formula - (With ties one can apply either Kerby or King Minium Formulae but not cureton - King's Formula is the most safe)
