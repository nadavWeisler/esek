...
def from_data(
    ...
    correction = np.sqrt(
        self.area_under_function(
            self.density, norm.ppf(trimming_level), norm.ppf(1 - trimming_level)
        )
    ...
    winsorized_variance_value = self.winsorized_variance(sort_values, trimming_level)
    ...
    concatenated_samples = [
        np.concatenate((x, y)) for x, y in zip(bootstrap_samples_x, bootstrap_samples_y)
    ]
    ...
    winsorized_variances_bootstrap = [
        self.winsorized_variance(arr, trimming_level) for arr in concatenated_samples
    ]
...
def area_under_function(
    self, f, a, b, *args, function_a=None, function_b=None, limit=10, eps=1e-5
):
...
def winsorized_variance(x, trimming_level=0.2):
    y = np.sort(x)
    n = len(x)
...
def winsorized_correlation(x, y, trimming_level=0.2):
    sample_size = len(x)
    x_sorted = np.sort(x)
    y_sorted = np.sort(y)
